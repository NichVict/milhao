...
import datetime
import time
from banco_dado_sl import inserir_preco_atingido, consultar_precos_atingidos
import sys
...
def enviar_notificacao(destinatario, assunto, corpo, remetente, senha_ou_token, token_telegram):
    ...
    mensagem_telegram = f"{corpo}\n\nAviso automático do Canal 1 milhao."
                    ...
                    if (operacao == 'compra' and preco_atual >= preco_alvo) or (
                        ...
                        if primeira_verificacao:
                            ...
                            if tempo_acumulado >= TEMPO_ACUMULADO_MAXIMO:
                                ...
                                # Adicione este print para verificar se os dados foram inseridos corretamente
                                print(f"Dados inseridos no banco de dados para {ticker_symbol} - Data Saída: {data_saida}, Stop Loss: {stop_loss}")
...
def criar_janela(token_telegram):
    ...
    janela = tk.Tk()
    janela.title("ORDENS DE STOP LOSS - MONITORAMENTO")
    ...
    for ativo, _, _, _ in ativos_e_precos:
        ...
        estados_tickers_var[ativo].set("0")  # Define o estado inicial como 0
    ...
    # Inicia a atualização do estado dos tickers na interface
    janela.after(60000, atualizar_estado_ticker_interface)
...
def atualizar_interface(output_text):
    ...
    # Chama a função novamente após 60 mil milisegundos (60 seg)
