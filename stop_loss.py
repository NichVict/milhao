...
def enviar_notificacao(destinatario, assunto, corpo, remetente, senha_ou_token, token_telegram):
    ...
    chat_id = '-1002094232063'  # Substitua pelo seu ID de chat do Telegram - DO GRUPO ALERTA TRADES
        ...
        while True:
            ...
            if (operacao == 'compra' and preco_atual >= preco_alvo) or (
                ...
                if ticker_symbol not in estados_tickers:
                    ...
                    imprimir_mensagem(
                        f"Ticker {ticker_symbol} atingiu o preço-alvo pela primeira vez!")
                    ...
                    if (operacao == 'compra' and preco_atual >= preco_alvo) or (
                        ...
                        if not primeira_verificacao:
                            ...
                            imprimir_mensagem(
                                f"Ticker {ticker_symbol} inicializando contagem de Tempo Acumulado...")

                        if primeira_verificacao:
                            # Incrementa o tempo acumulado apenas uma vez por intervalo de 60 segundos
                            #imprimir_mensagem(
            ...
            time.sleep(INTERVALO_VERIFICACAO)  # Aguarda 60 segundos entre as verificações
...
def notificar_preco_alvo_alcancado(ticker_symbol, preco_alvo, preco_atual, destinatario, operacao, token_telegram):
    ...
    if (operacao == 'compra' and preco_atual >= preco_alvo) or (operacao == 'venda' and preco_atual <= preco_alvo):
        mensagem = f"ENCERRAMENTO da Operaçao em {ticker_symbol_sem_extensao} foi ativada, conforme nossa Lista Semanal! STOP LOSS {preco_alvo:.2f} foi atingido ou ultrapassado. Preço atual: {preco_atual_formatado}\n\n\n\n\n"
...
def criar_janela(token_telegram):
    ...
    janela = tk.Tk()
    janela.title("OPERACOES STOP LOSS ALERTA TRADES XANDE")
...
def atualizar_interface(output_text):
    ...
    for ticker, estado in estados_tickers_var.items():
        output_text.config(state=tk.NORMAL)
        output_text.insert(tk.END, f"Ticker {ticker} sendo monitorado ...\n")
        output_text.config(state=tk.DISABLED)
        output_text.yview(tk.END)

    # Chama a função novamente após 1000 milissegundos (1 segundo)
    output_text.after(180000, lambda: atualizar_interface(output_text))
